...
with holy_hands.Hands(
    max_num_hands=1 # Here only one hand is going to be detect (We can change it if we want more hands to be detected)
                      ) as hands:
   ...
   while cap.isOpened():
     ...
     cv.imshow('Hand Gesture Detection', cv.flip(image,1))
